...
$current_year_month = date('Y-m'); // 'Y' is year, 'W' is week number
...
SELECT 
    ...
    fe.created_at
FROM food_entry fe
LEFT JOIN employee e ON fe.emp_id = e.emp_id
...
LEFT JOIN user_master u ON fe.created_by = u.id
WHERE DATE_FORMAT(DATE_SUB(fe.created_at, INTERVAL 4 HOUR), '%Y-%m') = '$current_year_month' 
ORDER BY fe.id ASC
";
}
else
...
LEFT JOIN user_master u ON fe.created_by = u.id
WHERE DATE_FORMAT(DATE_SUB(fe.created_at, INTERVAL 4 HOUR), '%Y-%m') = '$current_year_month' AND fe.created_by = '$vendor_id'  
ORDER BY fe.id ASC
";  
} 
					...
					<div class="ps-3">
						<nav aria-label="breadcrumb">
							<ol class="breadcrumb mb-0 p-0">
								...
								<li class="breadcrumb-item active" aria-current="page"><em> Monthly Meal List</em></li>
							</ol>
						</nav>
					</div>
                    ...
                    <div class="col-lg-12 mx-auto">
                        <div class="card">
                            <div class="card-header px-4 py-3">
                                <h5 class="mb-0 text-uppercase">Monthly Meal List</h5>
                            </div>
                            <div class="card-body p-4">
		                        <div id="rsttbl">
                                    <table id="example2" class="table table-striped table-bordered">
                                        ...
                                        <tbody>
                                            <?php
                                            if(count($food_entries) > 0) {
                                                $i = 1;
                                                foreach($food_entries as $entry) {

                                                  $foodBadge = '';

                                                    if ($entry['two_times_food_allowed'] == 1) {
                                                        $foodBadge = '<span class="badge bg-success">One Time</span>';
                                                    } elseif ($entry['two_times_food_allowed'] == 2) {
                                                    ...
                                                    echo "<tr>
                                                            <td>".$i++."</td>
                                                            <td>".htmlspecialchars($entry['emp_id'])."</td>
                                                            <td>".htmlspecialchars($entry['emp_name'])."</td>
...
<script>
    ...
    var table = $('#example2').DataTable({
        ...
        buttons:  [
                {
            extend: 'excel',
                title: 'Monthly Meal Employee List',
            exportOptions: {
