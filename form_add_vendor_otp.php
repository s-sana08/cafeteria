							...
							<div class="card-body p-4">
								<form id="frmvendor">
									...
									<div class="row mb-3">
										...
										<div class="col-md-9">
											<input type="text" class="form-control" id="vendoremailid" name="vendoremailid" placeholder="Enter Vendor Email ID">
										</div>
									</div>
                                    <div class="row mb-3">
...
<div class="row mb-3 d-none" id="otpSection">
    <label class="col-sm-3 col-form-label">Enter OTP</label>
    <div class="col-md-9 d-flex gap-2">
        <input type="text" id="txtotp" class="form-control" placeholder="Enter OTP">
        <button type="button" class="btn btn-success" id="btnVerifyOtp">Verify</button>
    </div>
</div>
									<div class="row mb-3">
...
<script>
 ...
 class EmployeeManager {
   ...
   e.preventDefault(); 
    ...
    Swal.fire({
        title: "Are you sure?",
        text: data.id ? "Do you want to update this vendor?" : "Do you want to add this vendor?",
        icon: "warning",
        ...
        if (result.isConfirmed) {
            ...
            $.post('api/add_vendor_operations_otp.php', data, (response) => {
                let res;
    ...
    showInEditor(id) {
        ...
        $('html, body').animate({ scrollTop: $('#frmvendor').offset().top - 200 }, 300);
    }
    ...
    loadDropdowns() {
        $.post('api/add_vendor_operations_otp.php', { flag: 'fetch_dropdowns' }, (response) => {
            const data = JSON.parse(response);
            let html = '<option value="">Select Food Type</option>';
            data.food_type_master.forEach(f => { html += `<option value="${f.id}">${f.food_type_name}</option>`; });
            this.$vndFood.html(html);
        });
    }

    // showData() {
    //     $.post('api/add_vendor_operations_otp.php', { flag: 'showdata' }, (response) => {
...
$(document).on('change', '.status-toggle', function() {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            $.post('api/add_vendor_operations_otp.php', { flag: 'toggle_status', id: id, status: status }, (response) => {
                let res;
                try {
...
$(document).ready(function () {
    ...
    $('#btnSendOtp').on('click', function () {
        ...
        $.post('api/add_vendor_operations_otp.php', {
            flag: 'send_otp',
            email: email
        }, function (res) {
            ...
            if (r.status === 'sent') {
                ...
                // ⚠️ Testing साठी OTP दाखवतो
                Swal.fire("OTP Sent", "Your OTP: " + r.otp, "success");
